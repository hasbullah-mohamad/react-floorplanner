...
function makeObjectMinLOD(RADIUS,HEIGHT) {
  ...
  let column = new Three.Mesh();
  let object = new Three.Mesh(new Three.CylinderGeometry(RADIUS, RADIUS, HEIGHT, 32, 32), frameMaterial);
    ...
    return (
      <g>
        ...
        <text key='2' cx='0' cy='0'
          ...
          {
            element.seats ? element.seats + ' ' + element.name : element.name
          }
        </text>
        {
          element.number && (
            <g key='3' style={numberStyle}>
              <circle key='4' cx={0} cy={RADIUS} r={20}></circle>
              <text key='5' x='0' y={6} transform={`translate(${0}, ${RADIUS}) scale(1,-1) rotate(${textRotation})`} style={{textAnchor: 'middle'}}>{element.number}</text>
            </g>
          )
        }
      </g>
